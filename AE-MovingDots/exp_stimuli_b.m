function exp_stimuli_b(ExpParams,Names,display,esound,dots,type)
...
for i = 1:numblocks
    ...
    m(1,:) = [f1.*ones(1,length(conds)*2) f2.*ones(1,length(conds)*2) zeros(1,length(conds)*2)];
    m(2,:) = [repmat(conds,1,2) repmat(conds,1,2) repmat(conds,1,2)];
    m(3,:) = ([repmat(1:length(conds),1,2) repmat(1:length(conds),1,2) repmat(1:length(conds),1,2)]);
