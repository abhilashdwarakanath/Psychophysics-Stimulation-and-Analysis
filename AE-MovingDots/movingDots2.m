function movingDots2(dots,display,redflag)
...
max_d       = 3.5;   % maximum radius of  annulus (degrees)
min_d       = 0;    % minumum
...
xymatrix = transpose(xy);
    ...
    reddotframe = round(1+(nframes-1)*rand);
    reddotr = (1+(rmax-1)*rand);
    reddottheta = 0+(2*pi-0)*rand;
    reddotxy=round([reddotr*cos(reddottheta);reddotr*sin(reddottheta)]);
...
for i = 1:nframes
    if (i>=1)
       Screen('FillOval', w, uint8(white), fix_cord);	% draw fixation dot (flip erases it)
        ...
        if i==reddotframe && redflag==1
        Screen('DrawDots',w,reddotxy+xymatrix_shift(:,1),10,[255 0 0],center,1);
        end
