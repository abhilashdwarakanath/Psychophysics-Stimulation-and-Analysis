function movingDots(display,dots)
...
%   Centre           [x,y] Centre of the aperture (degrees)
...
% Edited and changed by A. Dwarakanath at the MPI for biological
% Cybernetics, Germany.
...
for i=1:length(dots) %Loop through the fields
    ...
    l(i) = dots(i).Centre(1)-dots(i).apertureSize(1)/2;
    r(i) = dots(i).Centre(1)+dots(i).apertureSize(1)/2;
    b(i) = dots(i).Centre(2)-dots(i).apertureSize(2)/2;
    t(i) = dots(i).Centre(2)+dots(i).apertureSize(2)/2;
    
    %Generate random starting positions
    dots(i).x = (rand(1,dots(i).nDots)-.5)*dots(i).apertureSize(1) + dots(i).Centre(1);
    dots(i).y = (rand(1,dots(i).nDots)-.5)*dots(i).apertureSize(2) + dots(i).Centre(2);
    
    %Create a direction vector for a given coherence level
    direction = round(rand(1,dots(i).nDots)*360);
...
nFrames = round(dots.duration*display.frameRate);
...
try
    ...
    for frameNum=1:nFrames
        ...
        for i=1:length(dots)  %Loop through the fields
            ...
            % Determine which dots are within the circular aperture
             goodDots(id) = (dots(i).x-dots(i).Centre(1)).^2/(dots(i).apertureSize(1)/2)^2 + ...
                (dots(i).y-dots(i).Centre(2)).^2/(dots(i).apertureSize(2)/2)^2 < 1;
            ...
            dots(i).x(deadDots) = (rand(1,sum(deadDots))-.5)*dots(i).apertureSize(1) + dots(i).Centre(1);
            dots(i).y(deadDots) = (rand(1,sum(deadDots))-.5)*dots(i).apertureSize(2) + dots(i).Centre(2);
        ...
        %fixation(display,display.windowPtr);
        drawBlank(display);
    end
catch ME
    Screen('CloseAll');
